<template>
  <form
    ...
    <textarea ref="messageRef" v-model="questionText" rows="2" placeholder="Ask anything ... " @input="autoResize"
      ...
      style="min-height: 48px; max-height: 200px; overflow-y: auto;"></textarea>
    <!-- أدوات أسفل -->
    <div class="flex items-center justify-between ">
      <div class="flex items-center gap-2 bg-[#E7F1EF] w-fit rounded-lg">
        <img src="/search.png" alt="search" class="w-[36px] rounded-lg p-2 cursor-pointer"
          :class="{ 'border-2 border-[#21808D]': activeIcon === 'search' }" @click="setActiveIcon('search')" />
        <img src="/hourglass.png" alt="hourglass" class="w-[36px] rounded-lg p-2 cursor-pointer"
          :class="{ 'border-2 border-[#21808D]': activeIcon === 'hourglass' }" @click="setActiveIcon('hourglass')" />
        <img src="/flag.png" alt="flag" class="w-[36px] rounded-lg p-2 cursor-pointer"
          :class="{ 'border-2 border-[#21808D]': activeIcon === 'flag' }" @click="setActiveIcon('flag')" />
      </div>
      <div class="flex items-center gap-2">
        ...
        <div class="relative w-30 font-sans" ref="dropdownRef">
          ...
          <button @click="toggleDropdown" type="button"
            ...
            <span>Version 1</span>
            <svg class="w-4 h-4" :class="{ 'rotate-180': isOpen }" fill="none" stroke="currentColor"
              viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7" />
            </svg>
          </button>
          <!-- Dropdown Menu -->
          <div v-if="isOpen" ref="dropdownMenuRef" :class="[
            ...
            <div class="p-3 border-b border-gray-700 hover:bg-gray-700 transition flex justify-between items-center ">
            ...
            <div class="p-3 hover:bg-gray-700 transition flex justify-between items-center ">
              ...
              <button @click.stop="onToggleSwitch"
                ...
                <div class="w-3 h-3 bg-white rounded-full shadow-md transform transition duration-300  ease-in-out"
                  :class="{ 'translate-x-5': isToggled, 'translate-x-0': !isToggled }"></div>
              </button>
            </div>
          </div>
        </div>
